...
const levelColors = {
  CRITICAL: { bg: 'rgba(230,57,70,0.12)',  border: 'rgba(230,57,70,0.3)',  text: '#e63946' },
  HIGH:     { bg: 'rgba(244,162,97,0.12)', border: 'rgba(244,162,97,0.3)', text: '#f4a261' },
  LOW:      { bg: 'rgba(46,196,182,0.12)', border: 'rgba(46,196,182,0.3)', text: '#2ec4b6' },
};

function SkeletonCard() {
  return (
    <div style={{ ...s.card, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', animation: 'mlPulse 1.4s ease-in-out infinite' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
        <div style={skBar('40px', '16px')} />
        <div style={skBar('36px', '10px')} />
      </div>
      <div style={{ ...skBar('75%', '10px'), marginBottom: '16px' }} />
      <div style={{ ...skBar('100%', '5px'), borderRadius: '3px', marginBottom: '12px' }} />
...
const skBar = (w, h) => ({ width: w, height: h, borderRadius: '4px', background: 'rgba(255,255,255,0.08)' });
...
export default function MLPredictions() {
  ...
  // useEffect(() => {
  //   // Cache hit — skip the slow network call entirely
  //   if (getCached()) return;
  ...
  //     })
  //     .catch(err => {
  //       if (!axios.isCancel(err)) { setError(true); setLoading(false); }
  //     });
  ...
  return (
    <div style={s.wrapper}>
        ...
        <div style={s.titleRow}>
          ...
          {lastUpdated && !loading && (
            <span style={s.updated}>
              cached · {lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </span>
          )}
        </div>
        <p style={s.sub}>
      ...
      {!loading && (
        <div style={s.grid}>
          {predictions.filter(p => p.countryCode !== 'WW').map((p, i) => {
            const c = levelColors[p.riskLevel] || levelColors.LOW;
            return (
              <div key={i} style={{ ...s.card, background: c.bg, border: `1px solid ${c.border}` }}>
                <div style={s.cardTop}>
                  <span style={s.countryCode}>{p.countryCode}</span>
                  <span style={{ ...s.level, color: c.text }}>{p.riskLevel}</span>
                </div>
                <div style={s.region}>{p.region}</div>

                <div style={s.barLabel}>
                  <span style={s.barText}>Risk Score</span>
                  <span style={{ ...s.barNum, color: c.text }}>{p.riskScore}%</span>
                </div>
                <div style={s.track}>
                  <div style={{ ...s.fill, width: `${p.riskScore}%`, background: c.text }} />
                </div>

                <div style={s.cardBottom}>
                  <span style={s.stat}>
                    <AlertTriangle size={11} style={{ marginRight: '4px', flexShrink: 0 }} />
                    Avg: {p.avgSeverity}/10
                  </span>
                  <span style={s.stat}>
                    <Radio size={11} style={{ marginRight: '4px', flexShrink: 0 }} />
                    {p.eventCount} events
                  </span>
                </div>
              </div>
...
const s = {
  ...
  titleRow:   { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px', flexWrap: 'wrap' },
  dot:        { width: '8px', height: '8px', borderRadius: '50%', background: '#9b5de5', boxShadow: '0 0 8px #9b5de5', flexShrink: 0 },
  title:      { margin: 0, fontSize: '18px', fontWeight: '700', color: '#e8eaf0' },
  badge:      { display: 'flex', alignItems: 'center', padding: '3px 10px', borderRadius: '4px', fontSize: '10px', fontWeight: '700', letterSpacing: '1px', background: 'rgba(155,93,229,0.15)', color: '#9b5de5', border: '1px solid rgba(155,93,229,0.3)' },
  updated:    { fontSize: '10px', color: '#475569', fontFamily: "'JetBrains Mono', monospace" },
  ...
  countryCode:{ fontSize: '16px', fontWeight: '800', color: '#e8eaf0', letterSpacing: '1px' },
  level:      { fontSize: '10px', fontWeight: '700', letterSpacing: '1px' },
  region:     { fontSize: '12px', color: '#8892b0', marginBottom: '12px' },
  barLabel:   { display: 'flex', justifyContent: 'space-between', marginBottom: '4px' },
  barText:    { fontSize: '10px', color: '#8892b0', letterSpacing: '1px' },
  barNum:     { fontSize: '11px', fontWeight: '700' },
  track:      { height: '5px', background: 'rgba(255,255,255,0.1)', borderRadius: '3px', overflow: 'hidden', marginBottom: '12px' },
  fill:       { height: '100%', borderRadius: '3px' },
  cardBottom: { display: 'flex', justifyContent: 'space-between' },
  stat:       { fontSize: '11px', color: '#8892b0', display: 'flex', alignItems: 'center' },
};